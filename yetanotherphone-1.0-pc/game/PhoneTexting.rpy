...
define nvl_mode = "computer" ##Allow the NVL mode to become a computer conversation
...
init -1 python:
    ...
    computer_position_x = 0.5
    computer_position_y = 0.5
...
screen ComputerDialogue(dialogue, items=None):

    style_prefix "computerFrame"
    frame at phone_transform(computer_position_x, computer_position_y):
        if len(dialogue) == 1:
            at phone_appear(computer_position_x, computer_position_y)
        viewport:
            draggable True
            mousewheel True
            # cols 1
